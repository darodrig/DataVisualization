...
#Figure
def main(data):
    p = figure(plot_width=600, plot_height=600,title="Wait Time Daily")
#background fill color : Box Annotations

    p.add_layout(BoxAnnotation(bottom=3, top=10, fill_alpha=0.1, fill_color='#3FE258')) #GoodWT
    p.add_layout(BoxAnnotation(top=3, fill_alpha=0.1, fill_color='#E2793F')) #BadWT
    
    p.circle(x, y,size=15, color="#3F58E2", alpha=1)
