...
def main(data):
    ...
    if max(filter(None, source.data['y'])) >= 10: 
    ...
    p.line([0, source.data['class_duration'][0]+2], [3, 3], line_color='#3FE258', line_width=2, line_dash=[4, 4])  # ideal line

    p.circle(source.data['x'], source.data['y'], size=10, color="black", alpha=1, legend=("Content Question"))
    p.grid.grid_line_color = None
    p.background_fill_color = "white"
    p.y_range = y_axis_range
    p.x_range = Range1d(0, source.data['class_duration'][0]+2)
    p.xaxis.axis_label = "Class Duration (minutes)"
    p.yaxis.axis_label = "Wait Time (seconds)"

    # Date
    p.add_layout(Title(text="Data from %s" % source.data['date'][0], align="left", text_font_size="12px",
                          text_font_style="italic", text_alpha=.5), "below")

    if source.data['y']:
        WT_median = np.median(source.data['y'])
        if WT_median >= 3:
            WT_median_color = "#3FE258"
        else:
            WT_median_color = "#E2793F"

        p.line([0, source.data['class_duration'][0]+2], [WT_median, WT_median],
